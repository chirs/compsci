...
(defvar binaryfn '(defun ?function (tree) (if (consp tree) (?combine (?function (first tree)) (?function (rest tree))) ?baseanswer)))
...
(setq substitutions '(
       ...
       ( (?function copytree) (?combine cons) (?baseanswer (if (consp tree) tree '())))
       ( (?function mintree) (?combine min) (?baseanswer (if (numberp tree) tree 0)))
       ( (?function conses) (?combine add1) (?baseanswer (if (consp tree) tree 0)))
       ))
...
(redefpatterns 'derivpats '(
       ...
       ( (deriv (/ ?u ?v) ?x)  (+ (* ?v (deriv ?u ?x)) (/ (* ?u (deriv ?v ?x)) (* ?v ?v))))
       ...
       ( (deriv (tan ?u) ?x) (* (+ 1 (* (tan ?u) (tan ?u))) (deriv ?u ?X)))
...
(redefpatterns 'javapats '(
       ( (if ?test ?then ?other) ("if ( " ?test " )"  #\Tab #\Return ?then #\Tab #\Return ?other))
...
(defvar eqnslens)
(setq eqnslens '(
		 (= focal_length (+ subject_distance image_distance))
